...
FRONTEND_URL=http://localhost:9000/frontend/recv

# Where to put the Keychain db file
KEYCHAIN_DB_FOLDER=../data
KEYCHAIN_DB_FILE=mitm_client.db

# Uncomment this to specify your own config file folder and location
KEYCHAIN_CFG_FOLDER=../config
KEYCHAIN_CFG_FILE=keychain.cfg
KEYCHAIN_CFG_FILE_PATH=${KEYCHAIN_CFG_FOLDER}/${KEYCHAIN_CFG_FILE}


# CHANGE THIS TO THE SAME VALUE FOR ALL APPS
TD_SUFFIX=abcde
...
echo "Running with '-Dwith.keychain'"
WITH_KEYCHAIN="-Dwith.keychain"
...
${JAVA} \
    ...
    -Dkeychain.trusted.directory.suffix=${TD_SUFFIX} \
    -Dkeychain.db.file=${KEYCHAIN_DB_FILE_PATH} \
    -Dkeychain.cfg.file=${KEYCHAIN_CFG_FILE_PATH} \
    ${WITH_KEYCHAIN} \
